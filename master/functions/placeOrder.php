...
//    $user_id = $_SESSION['auth_user']['user_id'];
   ...
   $credit_information	 = isset($_POST['method']) ? 'Name('. $_POST['card_Name'] .'),card Number('. $_POST['card_Number'] .'),expiration Date('. $_POST['expiration_Date'] .'),CVV('. $_POST['CVV'].')':'0';  
   // 'Name('. $_POST['card_Name'] .'),card Number('. $_POST['card_Number'] .'),expiration Date('. $_POST['expiration_Date'] .'),CVV('. $_POST['CVV'].')';
   ...
   {      
    $sql = "INSERT INTO `orders`(`order_id`, `user_id`, `fullName`, `email`, `phone`, `delivery_time`, `letter`, `location`, `pay_method`, `credit_information` ,`total_products`, `total_price`, `order_time`) VALUES (NULL,'$user_id','$fullName','$email','$phone','$delivery_time','$letter','$location','$pay_method','$credit_information','$total_products','$total_price','$order_time');";
    $check_cart = mysqli_query($con , $sql) ;
